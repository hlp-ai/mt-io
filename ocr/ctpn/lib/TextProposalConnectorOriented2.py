...
from .text_proposal_graph_builder2 import TextProposalGraphBuilder, sub_graphs_connected
...
class TextProposalConnectorOriented:
    ...
    def get_text_lines(self, text_proposals, scores, im_size):
        ...
        for index, tp_indices in enumerate(tp_groups):
            ...
            # 多项式拟合
            lt_y, rt_y = self.fit_y(text_line_boxes[:, 0], text_line_boxes[:, 1], tf.cast(x0, tf.float32) + offset,
                                    tf.cast(x1, tf.float32) - offset)
            lb_y, rb_y = self.fit_y(X, text_line_boxes[:, 3], x0 + tf.cast(offset, tf.int32),
                                    x1 - tf.cast(offset, tf.int32))
