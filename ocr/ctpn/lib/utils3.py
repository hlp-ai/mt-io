...
def gen_anchor(featuresize, scale):
    ...
    h, w = featuresize
    shift_x = tf.cast(tf.range(0, w) * scale, dtype=tf.float32)  # 将shift_x转换为浮点数类型
    shift_y = tf.cast(tf.range(0, h) * scale, dtype=tf.float32)  # 将shift_y转换为浮点数类型
    ...
    anchor = tf.expand_dims(base_anchor, axis=0) + tf.expand_dims(shifts, axis=1)
